## SVG EDITOR

This is a program that creates a .svg file, allowing you to create shapes, modify their data and delete them. You can view your shape(s) in the file. The shapes are automatically saved in the file.

### CREATE A SHAPE

The user can choose to create a shape between the 5 following shapes: a line, an ellipse (including circles), a rectangle (including squares), a polyline, a polygone, and in bonus, a premade Patrick from SquarePants SpongeBob. The user will have to enter the coordinates of the points for the shape chosen. Patrick cannot be changed. The user can customise the color of the fill of the shapes, the width of the strokes and their color.

### MODIFY A SHAPE

The user can choose to modify any shape they made, by entering the new data points. The user can also change the personalisation of the shapes.

### DELETE A SHAPE

The user can choose to delete any shape they made.

### VISUALISATION

The user can visualise the shapes they made in the .svg file. 

### TO COME

The user will be able to pivot the shape. 

